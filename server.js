...
function compliancePage(type) {
  ...
  if (type === 'privacy') return '<!DOCTYPE html><html><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0"><title>Privacy Policy</title>' + style + '</head><body><h1>Privacy Policy</h1><p><strong>Effective:</strong> April 2026 &middot; <strong>Last updated:</strong> April 2026 &middot; <strong>Campaign:</strong> Luis for Port of Brownsville Place 4 (the &ldquo;Campaign&rdquo;)</p><h2>1. Information We Collect</h2><p><strong>From you directly:</strong> name, phone number, email, mailing address &mdash; when you sign up via our form, text a keyword, or speak with a campaign volunteer.</p><p><strong>From public records:</strong> We use the publicly available Texas voter file (Texas Election Code &sect;18.001 et seq.) to identify registered voters in our district for door-to-door canvassing and SMS outreach. This includes name, mailing address, voter status, voting history (which elections a person voted in, not how they voted), and party affiliation where stated.</p><p><strong>From our apps (CampaignText, BlockWalker):</strong> When campaign staff and volunteers use our iPhone apps, we may access (with your permission) your device&rsquo;s location during a block walk, your microphone for voice notes, your camera for QR check-in, and your contacts solely to match against the voter file on-device. <strong>Your contacts are never uploaded to our servers.</strong></p><h2>2. How We Use Your Information</h2><ul><li>Send campaign updates, event invitations, and election reminders via SMS, MMS, and email</li><li>Provide voting information (polling locations, early voting dates, ID requirements)</li><li>Coordinate door-to-door canvassing</li><li>Track engagement to improve our outreach</li></ul><h2>3. Service Providers (Sub-Processors)</h2><p>We use the following vendors to operate the program. Each is bound by confidentiality and uses your information only to provide service to the Campaign:</p><ul><li><strong>RumbleUp</strong> &mdash; SMS/MMS broadcast delivery</li><li><strong>Twilio</strong> &mdash; SMS delivery and webhook receipt</li><li><strong>Anthropic (Claude AI)</strong> &mdash; reply summarization and sentiment classification (no message content is used to train models)</li><li><strong>Google</strong> &mdash; Sheets sync (campaign-internal only) and trend lookups</li><li><strong>Railway</strong> &mdash; web hosting</li><li><strong>Apple TestFlight / App Store</strong> &mdash; iOS app distribution</li></ul><h2>4. Text Messaging Program</h2><p>By opting in, you consent to receive recurring campaign text messages. Message frequency varies (typically up to 10 per month, more in the final 30 days before an election). Message and data rates may apply. <strong>Reply STOP at any time to opt out</strong> &mdash; you will be removed within minutes. Reply HELP for assistance.</p><h2>5. Information Sharing</h2><p>We do <strong>not</strong> sell your personal information. We do not share it with other campaigns or with third parties for their marketing. We may share with: (a) campaign staff and volunteers operating under confidentiality; (b) the service providers listed above, only as needed to run the program; (c) law enforcement if compelled by valid legal process.</p><h2>6. Data Retention &amp; Deletion</h2><p>We retain campaign records for the duration of the campaign cycle plus any period required by Texas Ethics Commission rules. After that, contact records are deleted or de-identified. <strong>You can request deletion at any time</strong> by texting STOP (which removes you from outreach) or by emailing the Campaign at the address below for full deletion of your record.</p><p><strong>App users:</strong> Captains and walkers who use the apps can delete their account directly from the in-app Account menu.</p><h2>7. Your Rights</h2><p>If you live in California, the EU, or another jurisdiction with privacy laws, you have the right to: know what we hold about you, ask for a copy, ask for correction or deletion, and opt out of any sale (we do not sell). Email the Campaign to exercise these rights.</p><h2>8. Children</h2><p>This service is not directed to anyone under 18. We do not knowingly collect information from minors. If you believe a minor has provided information, contact us and we will delete it.</p><h2>9. Security</h2><p>We use TLS for transit, encrypted storage at rest, and access controls limiting voter data to authorized campaign staff. No system is 100% secure; we will notify affected users in the event of a material breach as required by law.</p><h2>10. Changes</h2><p>We may update this policy. Material changes will be posted here with a new &ldquo;Last updated&rdquo; date.</p><h2>11. Contact</h2><p>To exercise any privacy right described above, request deletion of your data, or ask a question about this policy, contact us:</p><p><strong>Email:</strong> <a href="mailto:privacy@campaigntext.app">privacy@campaigntext.app</a><br><strong>Mail:</strong> Luis for Port of Brownsville Place 4 &middot; Brownsville, TX</p><p>We respond to verified privacy requests within 30 days.</p><p style="margin-top:32px;font-size:12px;color:#94a3b8"><a href="/terms">Terms of Service</a></p></body></html>';
...
app.use((req, res, next) => {
  ...
  if (req.path.match(/^\/api\/p2p\/join/) ||
      ...
      req.path === '/api/volunteers/me/delete' ||
  ...
  if (req.path.match(/^\/api\/captains\/login/) ||
      ...
      req.path === '/api/captains/me/delete' ||
      req.path.match(/^\/api\/captains\/\d+\/text-log$/) ||
      req.path.match(/^\/api\/captains\/\d+\/phone-log$/) ||
      req.path.match(/^\/api\/captains\/\d+\/voters\/\d+\/call-history$/) ||
      ...
      req.path.match(/^\/api\/captains\/\d+\/add-phone$/) ||
      req.path.match(/^\/api\/captains\/\d+\/voters\/\d+\/phone-reassign$/) ||
      req.path.match(/^\/api\/captains\/\d+\/voters\/\d+\/phone-mark-wrong$/) ||
...
app.post('/incoming', webhookLimiter, async (req, res) => {
  ...
  // TCPA / CTIA Common Short Code requirement: any messaging program that
  // tells users "Reply HELP for help" MUST actually answer HELP. A missing
  // response is one of the fastest ways to get carriers to flag/throttle
  // your number. Keep it short — 160 chars max for a single SMS segment.
  if (msgText === 'help' || msgText === 'info') {
    const helpReply = "Luis for Port of Brownsville Place 4 campaign. Msg & data rates may apply. Reply STOP to opt out. Questions: campaigntext-production.up.railway.app/privacy";
    return res.type(replyType).send(provider.buildReply(helpReply));
  }
