  ...
  if (mediaUrl) {
    if (creds.phoneNumber) {
      ...
      try {
        ...
        console.error('[rumbleup] MMS /proxy/send FAILED:', proxyErr.message);
        // Don't fall back silently — throw so the caller knows MMS failed
        // Only fall back if it's a permissions issue (403)
        if (proxyErr.message && proxyErr.message.includes('403')) {
          console.warn('[rumbleup] Falling back to SMS (no MMS permissions)');
          payload.text = body + '\n\nView your event flyer: ' + mediaUrl;
        } else {
          throw proxyErr; // Re-throw non-permission errors
        }
      }
    } else {
