...
async function sendSms(to, body, mediaUrl) {
  ...
  // MMS: attach image URL if provided (RumbleUp supports .png, .jpg, .gif, max 750KB)
  // Try both 'file' and 'media' params — RumbleUp docs show 'file' in message log but may use 'media' for sends
  if (mediaUrl) {
    payload.file = mediaUrl;
    payload.media = mediaUrl;
    console.log('[rumbleup] Sending MMS to ' + phone + ' with image: ' + mediaUrl);
  }
  const result = await apiPost('/message/send', payload);
  if (mediaUrl) console.log('[rumbleup] MMS send result:', JSON.stringify(result).substring(0, 500));
  return result;
}
