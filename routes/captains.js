...
// ─── MERGE source captain list INTO target captain list ───
// Both lists must belong to the calling captain.  Sub-captain access through
// requireCaptainAuth is intentionally NOT enough for this — merging is
// destructive on the source if delete_source=true, so we only allow the
// list's actual owner.  Body: { target_list_id, delete_source }.
router.post('/captains/:id/lists/:listId/merge-into', requireCaptainAuth, (req, res) => {
  const captainId = parseInt(req.params.id, 10);
  const sourceId = parseInt(req.params.listId, 10);
  ...
  // Ownership check — captain must own BOTH lists (no cross-captain merging).
  const source = db.prepare('SELECT id, name FROM captain_lists WHERE id = ? AND captain_id = ?').get(sourceId, captainId);
  const target = db.prepare('SELECT id, name FROM captain_lists WHERE id = ? AND captain_id = ?').get(targetId, captainId);
  if (!source) return res.status(404).json({ error: 'Source list not found or not yours.' });
  if (!target) return res.status(404).json({ error: 'Target list not found or not yours.' });

  const sourceCount = db.prepare('SELECT COUNT(*) as c FROM captain_list_voters WHERE list_id = ?').get(sourceId).c;
  const overlap = db.prepare(`
    SELECT COUNT(*) as c
    FROM captain_list_voters a
    WHERE a.list_id = ? AND a.voter_id IN (SELECT voter_id FROM captain_list_voters WHERE list_id = ?)
  `).get(sourceId, targetId).c;

  const tx = db.transaction(() => {
    const r = db.prepare(`
      INSERT OR IGNORE INTO captain_list_voters (list_id, voter_id)
      SELECT ?, voter_id FROM captain_list_voters WHERE list_id = ?
    `).run(targetId, sourceId);
    let removedSource = 0;
    if (deleteSource) {
      db.prepare('DELETE FROM captain_list_voters WHERE list_id = ?').run(sourceId);
      const dr = db.prepare('DELETE FROM captain_lists WHERE id = ? AND captain_id = ?').run(sourceId, captainId);
...
// Remove early voters from a captain list (in-place destructive cleanup).
router.post('/captains/:id/lists/:listId/remove-voted', requireCaptainAuth, (req, res) => {
  const captainId = parseInt(req.params.id, 10);
  const listId = parseInt(req.params.listId, 10);
  const list = db.prepare('SELECT id, name FROM captain_lists WHERE id = ? AND captain_id = ?').get(listId, captainId);
  if (!list) return res.status(404).json({ error: 'List not found or not yours.' });
  const before = db.prepare('SELECT COUNT(*) as c FROM captain_list_voters WHERE list_id = ?').get(listId).c;
  const r = db.prepare(`
    DELETE FROM captain_list_voters
    WHERE list_id = ?
      AND voter_id IN (SELECT id FROM voters WHERE early_voted = 1)
  `).run(listId);
  const after = db.prepare('SELECT COUNT(*) as c FROM captain_list_voters WHERE list_id = ?').get(listId).c;
  res.json({ success: true, removed: r.changes, before, after, listName: list.name });
});

// Preview voted-count for a captain list (used by the confirm dialog).
router.get('/captains/:id/lists/:listId/voted-count', requireCaptainAuth, (req, res) => {
  const captainId = parseInt(req.params.id, 10);
  const listId = parseInt(req.params.listId, 10);
  const list = db.prepare('SELECT id FROM captain_lists WHERE id = ? AND captain_id = ?').get(listId, captainId);
  if (!list) return res.status(404).json({ error: 'List not found or not yours.' });
  const total = db.prepare('SELECT COUNT(*) as c FROM captain_list_voters WHERE list_id = ?').get(listId).c;
  const voted = db.prepare(`
    SELECT COUNT(*) as c
    FROM captain_list_voters clv
    JOIN voters v ON clv.voter_id = v.id
    WHERE clv.list_id = ? AND v.early_voted = 1
  `).get(listId).c;
