...
// Missing age is common in real voter files (voter file gaps) and contact entries (captain
// might skip age). Treating a missing age as 0 would clamp ageScore to 0 and silently penalize
// otherwise-perfect name matches, so we drop the age component and renormalize the remaining
// weights (0.5/0.3 → 0.625/0.375) when either side's age is null/undefined.
function scoreCandidate(contact, voter) {
  ...
  const hasContactAge = contact.age !== null && contact.age !== undefined;
  const hasVoterAge = voter.age !== null && voter.age !== undefined;
  if (!hasContactAge || !hasVoterAge) {
    return 0.625 * lastNameScore + 0.375 * firstNameScore;
  }
  const ageGap = Math.abs(contact.age - voter.age);
